...
<body>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 p-0 overflow-auto" style="height: 88vh">
                <div
                    ...
                    <h1 class="h3">Edit Encode</h1>
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#uploadExcel">
                        Upload Excel
                    </button>
                ...
                <div
                        ...
                        <div class="modal-dialog modal-dialog-centered modal-lg">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <button type="submit" name="saveUpload" class="btn btn-primary mt-3">Upload</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    <form class="row g-3" action="../php/encodeAccount.php" onsubmit="return isvalid()" method="POST">
                        ...
                        <div class="col-12">
                            <button name="editEncodeAccount" id="editEncodeAccount" type="submit"
    ...
    <script>
        // Fetch data from the PHP file using JavaScript
        var urlParams = new URLSearchParams(window.location.search);
        var id = urlParams.get('id');

        fetch('../php/accountSelect.php?id=' + id)
            .then(response => response.json())
            .then(data => {
                // Format the date as needed
                var formattedCallDate = formatDate(data.callDate);
                var formattedStartContractDate = formatDate(data.startContractDate);
                var formattedEndContractDate = formatDate(data.endContractDate);

                // Fill out the form with the retrieved data
                $("#id").val(data.id);
                $("#accountExecutive").val(data.accExec);
                $("#accountName").val(data.accName);
                $("#callDate").val(formattedCallDate);
                $("#endUser").val(data.endUser);
                $("#address").val(data.address);
                $("#area").val(data.area);
                $("#accountCategory").val(data.accCat);
                $("#segment").val(data.segment);
                $("#industry").val(data.industry);
                $("#accountSource").val(data.accSource);
                $("#contactPerson").val(data.contactPerson);
                $("#designation").val(data.designation);
                $("#contactNumber").val(data.contactNumber);
                $("#emailAddress").val(data.email);
                $("#decisionMaker").val(data.decisionMaker);
                $("#dmContactNumber").val(data.dmNumber);
                $("#dmDesignation").val(data.dmDesignation);
                $("#existingSystem").val(data.existingSystem);
                $("#contractType").val(data.contactType);
                $("#contractStartDate").val(formattedStartContractDate);
                $("#contractEndDate").val(formattedEndContractDate);
                $("#proposedSystem").val(data.proposedSystem);
                $("#proposedPrice").val(data.proposedPrice);
                $("#paymentTerms").val(data.paymentTerms);
                $("#callNature").val(data.callNature);
                $("#accountStatus").val(data.accStatus);
                $("#followUpAction").val(data.actionFollow);
                $("#whatTranspired").val(data.whatTranspired);
        ...
        function formatDate(sqlDate) {
            if (!sqlDate || sqlDate === '0000-00-00') {
                // Handle invalid or empty dates here
                return ''; // or return a placeholder like 'N/A'
            }

            // Assuming sqlDate is in "yyyy-mm-dd" format
            var parts = sqlDate.split("-");
            if (parts.length === 3) {
                return parts[0] + "-" + parts[1] + "-" + parts[2];
            } else {
                // Handle unexpected date format
                console.error("Unexpected date format:", sqlDate);
